...
GUILD_ID = os.getenv("GUILD_ID") # Não usado diretamente no código, mas mantido para referência
...
CANAL_ID_HOSPEDAGEM = int("1386760995627602103") # ID do canal onde o status é exibido
CANAL_ID_LOGS = int("1386763639373041675") # ID do canal para logs
...
filas = {nome: asyncio.Queue() for nome in recursos} # Cada recurso tem sua própria fila
...
class BotaoDesconectar(discord.ui.View):
    ...
    def __init__(self, recurso):
        ...
        button = discord.ui.Button(label="🔌 Desconectar", style=discord.ButtonStyle.red, custom_id=f"desconectar_{recurso}")
    ...
    async def desconectar_button(self, interaction: discord.Interaction):
        ...
        try:
            if recursos[self.recurso] == usuario:
                ...
                await deletar_canal_temporario(usuario, self.recurso) # Deleta o canal de conexão ativa
                await atualizar_status() # Atualiza o status principal
                await verificar_fila(self.recurso) # Verifica a fila após liberação
            else:
            ...
            print("⚠️ Interação expirada ou canal não encontrado, fazendo cleanup silencioso.")
            if recursos[self.recurso] == usuario:
                recursos[self.recurso] = None
                cancelar_timer(self.recurso)
                await logar(f"{usuario.mention} desconectou do **{self.recurso}** via botão (cleanup silencioso).")
                await deletar_canal_temporario(usuario, self.recurso)
                await atualizar_status()
                await verificar_fila(self.recurso)
        except Exception as e:
            print(f"❌ Erro ao desconectar via botão para {usuario.name} do {self.recurso}: {e}")
            if not interaction.response.is_done():
                await interaction.response.send_message("❌ Ocorreu um erro ao desconectar. Tente novamente.", ephemeral=True)
...
class ConfirmarFilaView(discord.ui.View):
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.usuario_id:
            await interaction.response.send_message("🚫 Esta interação não é para você.", ephemeral=True)
            return
        self.value = True
        self.stop()
        await interaction.response.edit_message(content="✅ Você optou por entrar na fila.", view=None)
    ...
    async def cancel_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.value = False
        self.stop()
        await interaction.response.edit_message(content="❌ Você optou por não entrar na fila.", view=None)
...
class QueueThreadView(discord.ui.View):
    ...
    def __init__(self, recurso, usuario_id):
        ...
        button = discord.ui.Button(label="🚶 Sair da Fila", style=discord.ButtonStyle.red, custom_id=f"sairfila_btn_{recurso}_{usuario_id}")
    ...
    async def sair_fila_button(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if usuario.id in fila_atual:
                ...
                await interaction.response.send_message(f"❌ Você saiu da fila para **{self.recurso}**.", ephemeral=True)
                await logar(f"{usuario.mention} saiu da fila para **{self.recurso}** via botão na thread.")
                await deletar_canal_fila_temporario(usuario, self.recurso) # Deleta a thread da fila
                await atualizar_status()
            else:
                await interaction.response.send_message(f"Você não está mais na fila para **{self.recurso}**.", ephemeral=True)
        except Exception as e:
            print(f"❌ Erro ao sair da fila via botão para {usuario.name} do {self.recurso}: {e}")
            if not interaction.response.is_done():
                await interaction.response.send_message("❌ Ocorreu um erro ao sair da fila. Tente novamente.", ephemeral=True)
    ...
    async def select_callback(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        try:
            if recursos[recurso] is None:
                ...
                await criar_canal_temporario(usuario, recurso) # Cria a thread de conexão ativa
                ...
                await deletar_canal_temporario(usuario, recurso) # Deleta a thread de conexão ativa
                await verificar_fila(recurso) # Verifica a fila após desconexão
            else:
                # Recurso em uso por outra pessoa, oferecer fila
                ocupante = recursos[recurso]
                ocupante_mention = ocupante.mention if hasattr(ocupante, 'mention') else "outro usuário"

                # Verificar se o usuário já está na fila
                fila_atual = list(filas[recurso]._queue) # Acessar a fila interna (não recomendado para modificação, mas ok para leitura)
                if usuario.id in fila_atual:
                    await interaction.response.send_message(f"Você já está na fila para **{recurso}**. Sua posição: {fila_atual.index(usuario.id) + 1}.", ephemeral=True)
                ...
                if view.value is True:
                    await filas[recurso].put(usuario.id) # Adiciona o ID do usuário à fila
                    posicao_na_fila = list(filas[recurso]._queue).index(usuario.id) + 1
                    await logar(f"{usuario.mention} entrou na fila para **{recurso}**. Posição: {posicao_na_fila}")
                    # NOVO: Cria a thread da fila em vez de enviar mensagem efêmera
                    await criar_canal_fila_temporario(usuario, recurso, posicao_na_fila)
                    await interaction.followup.send(f"✅ Você entrou na fila para **{recurso}**. Verifique seu canal temporário de fila.", ephemeral=True)
                elif view.value is False:
                    await interaction.followup.send(f"Você optou por não entrar na fila para **{recurso}**.", ephemeral=True)
                else: # Timeout
                    await interaction.followup.send(f"Tempo esgotado. Você não entrou na fila para **{recurso}**.", ephemeral=True)

            await atualizar_status() # Sempre atualizar o status após qualquer mudança de conexão/fila
        except Exception as e:
            print(f"❌ Erro no select_callback para {usuario.name} e {recurso}: {e}")
            if not interaction.response.is_done():
    ...
    try:
        ...
        if msg_id:
            try:
                ...
                print(f"❌ Erro ao editar mensagem fixa: {edit_error}. Tentando criar nova.")
                nova_msg = await canal.send(content=conteudo, view=view)
                await nova_msg.pin()
        else:
...
async def logar(mensagem):
    ...
    try:
        ...
        if canal:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] {mensagem}") # Imprime no console mesmo sem canal de logs
    except Exception as e:
        print(f"❌ Erro ao enviar log: {e}")
        print(f"[{datetime.now().strftime('%H:%M:%S')}] {mensagem}") # Imprime no console em caso de erro
...
def iniciar_timer(recurso):
    ...
    async def desconectar_apos_tempo():
        ...
        if recursos[recurso] and hasattr(recursos[recurso], 'mention'):
            ...
            recursos[recurso] = None
            await atualizar_status()
            await verificar_fila(recurso) # Verifica a fila após desconexão por tempo
...
async def criar_canal_temporario(usuario: discord.User, recurso: str):
    ...
    try:
        ...
        print(f"❌ Erro ao criar thread temporária de conexão para {usuario.name} e {recurso}: {e}")
...
async def deletar_canal_temporario(usuario: discord.User, recurso: str):
    ...
    try:
        ...
        print(f"❌ Erro ao deletar thread temporária de conexão para {usuario.name} e {recurso}: {e}")

# NOVO: Funções para gerenciar threads de fila
async def criar_canal_fila_temporario(usuario: discord.User, recurso: str, posicao: int):
    ...
    try:
        ...
        print(f"❌ Erro ao criar thread temporária de fila para {usuario.name} e {recurso}: {e}")
...
async def deletar_canal_fila_temporario(usuario: discord.User, recurso: str):
    ...
    try:
        ...
        print(f"❌ Erro ao deletar thread temporária de fila para {usuario.name} e {recurso}: {e}")
...
async def verificar_fila(recurso: str):
    ...
    if not filas[recurso].empty():
        proximo_usuario_id = await filas[recurso].get() # Remove o primeiro da fila
        try:
            ...
            if proximo_usuario:
                ...
                if recursos[recurso] is None:
                    ...
                    iniciar_timer(recurso)
                    await criar_canal_temporario(proximo_usuario, recurso) # Cria a thread de conexão ativa
                    await deletar_canal_fila_temporario(proximo_usuario, recurso) # NOVO: Deleta a thread da fila
                    ...
                    await logar(f"🔔 {proximo_usuario.mention} foi automaticamente conectado a **{recurso}** da fila.")
                    await atualizar_status() # Atualiza o status após a conexão automática
                else:
                    # Cenário de corrida: recurso foi ocupado por outro antes da conexão da fila
                    await logar(f"⚠️ Recurso {recurso} foi ocupado por outro antes que {proximo_usuario.mention} pudesse ser conectado da fila.")
                    await proximo_usuario.send(
                ...
                await logar(f"⚠️ Usuário com ID {proximo_usuario_id} não encontrado (provavelmente saiu do servidor) para conexão automática da fila de {recurso}.")
            ...
            await logar(f"❌ Erro ao tentar conectar usuário da fila para {recurso}: {e}")
            await proximo_usuario.send(
            ...
            await verificar_fila(recurso) # Verifica a fila novamente em caso de erro
...
async def iniciaruso(interaction: discord.Interaction, recurso: str):
    ...
    if recursos[recurso] is not None:
        ...
        if interaction.user.id in fila_atual:
            await interaction.response.send_message(f"Você já está na fila para **{recurso}**. Sua posição: {fila_atual.index(interaction.user.id) + 1}.", ephemeral=True)
            return
        ...
        if view.value is True:
            await filas[recurso].put(interaction.user.id) # Adiciona o ID do usuário à fila
            posicao_na_fila = list(filas[recurso]._queue).index(usuario.id) + 1 # Posição correta
            await logar(f"{usuario.mention} entrou na fila para **{recurso}**. Posição: {posicao_na_fila}")
            # NOVO: Cria a thread da fila em vez de enviar mensagem efêmera
            await criar_canal_fila_temporario(usuario, recurso, posicao_na_fila)
            await interaction.followup.send(f"✅ Você entrou na fila para **{recurso}**. Verifique seu canal temporário de fila.", ephemeral=True)
...
async def encerraruso(interaction: discord.Interaction, recurso: str):
    ...
    recursos[recurso] = None
    cancelar_timer(recurso)
    await deletar_canal_temporario(interaction.user, recurso) # Deleta a thread de conexão ativa
...
async def entrarfila(interaction: discord.Interaction, recurso: str):
    ...
    if recursos[recurso] == interaction.user:
        await interaction.response.send_message(f"Você já está conectado a **{recurso}**.", ephemeral=True)
        return
    ...
    await interaction.response.send_message(f"✅ Você entrou na fila para **{recurso}**. Sua posição atual: {posicao_na_fila}.", ephemeral=True)
    await logar(f"{interaction.user.mention} entrou na fila para **{recurso}** via comando. Posição: {posicao_na_fila}")
    # NOVO: Cria a thread da fila em vez de enviar mensagem efêmera
...
async def sairfila(interaction: discord.Interaction, recurso: str):
    ...
    if interaction.user.id not in fila_atual:
        await interaction.response.send_message(f"Você não está na fila para **{recurso}**.", ephemeral=True)
        return
    ...
    await interaction.response.send_message(f"❌ Você saiu da fila para **{recurso}**.", ephemeral=True)
    await logar(f"{interaction.user.mention} saiu da fila para **{recurso}** via comando.")
    await deletar_canal_fila_temporario(interaction.user, recurso) # NOVO: Deleta a thread da fila
    await atualizar_status()
